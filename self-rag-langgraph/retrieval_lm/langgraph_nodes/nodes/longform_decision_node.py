...
The long-form version generates 25 tokens and checks if [Retrieval] appears in the text,
then uses logprob ratio if threshold is set.
"""
...
def build_longform_decision_node(model, ret_tokens: Dict[str, int], config):
    ...
    def longform_decision_node(state):
        ...
        if mode == "no_retrieval":
            return {**state, "do_retrieve": False}
        if mode == "always_retrieve":
            return {**state, "do_retrieve": True}
        ...
        return {**state, "do_retrieve": bool(do_retrieve)}
